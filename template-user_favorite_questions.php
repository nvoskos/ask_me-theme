...
get_header();
	...
	<div class="page-content page-content-user">
		<div class="user-questions">
			...
			$_favorites = get_user_meta($user_login->ID,$user_login->user_login."_favorites",true);
			$paged = (get_query_var("paged") != ""?(int)get_query_var("paged"):(get_query_var("page") != ""?(int)get_query_var("page"):1));
			if (isset($_favorites) && is_array($_favorites) && count($_favorites) >= 1) {
				$get_current_user_id = get_current_user_id();
				if (is_super_admin($get_current_user_id)) {
					$user_id_array = array();
				}else {
					$user_id_array = array("key" => "user_id","compare" => "NOT EXISTS");
					if (is_user_logged_in) {
						$user_id_array = array(
							'relation' => 'OR',
							array('relation' => 'AND',array("key" => "user_id","compare" => "!=","value" => $get_current_user_id),array("key" => "comment_count","compare" => ">","value" => 0)),
							array("key" => "user_id","compare" => "=","value" => $get_current_user_id),
							$user_id_array
						);
					}
				}
				$meta_query = array("meta_query" => array($user_id_array));
				$args = array_merge($meta_query,array('post_type' => 'question','posts_per_page' => $rows_per_page,'paged' => $paged,'post__in' => $_favorites));
				query_posts($args);
	...
	<?php if (isset($_favorites) && is_array($_favorites) && count($_favorites) >= 1) {
