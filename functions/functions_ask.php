...
function report_q () {
	...
	sendEmail(get_bloginfo("admin_email"),get_bloginfo('name'),vpanel_options("email_template"),get_bloginfo('name'),$email_title,$last_message_email);
	if ($user_id > 0) {
	...
	if ($active_notified == 1) {
		...
		if ($remember_answer == 1 && $post_author != $comment_user_id) {
			...
			if ($the_mail != "") {
				...
				sendEmail(get_bloginfo("admin_email"),get_bloginfo('name'),$the_mail,$the_author,$email_title,$last_message_email);
			}
		}
	}
	...
	if ($question_follow == 1 && isset($following_questions) && is_array($following_questions)) {
		...
		foreach ($following_questions as $user) {
			if ($user_id_question != $user) {
				...
				sendEmail(get_bloginfo("admin_email"),get_bloginfo('name'),esc_attr($all_meta_for_user->user_email),esc_attr($all_meta_for_user->display_name),$email_title,$last_message_email);
			}
		}
	}
					...
					if (is_super_admin($another_user_id) && ($private_question == "on" || $private_question == 1) && (($another_user_id != $anonymously_user && $anonymously_user > 0) || ($another_user_id != $not_user && $not_user > 0))) {
						...
						if ($not_user != $another_user_id && (empty($user_group) || (is_array($user_group) && is_array($get_capabilities) && isset($user_group[key($get_capabilities)]) && $user_group[key($get_capabilities)] == 1))) {
							...
							if ($yes_private == 1) {
								...
								sendEmail(get_bloginfo("admin_email"),get_bloginfo('name'),esc_attr($value->user_email),esc_attr($value->display_name),$email_title,$last_message_email);
							}
						}
					}
				}
   			...
   			if (get_post_type($return) == "question") {
	   			...
	   			if ($question_publish == "draft" && !is_super_admin($user_id)) {
					...
					if ($send_email_draft_questions == 1) {
						...
						sendEmail(get_bloginfo("admin_email"),get_bloginfo('name'),get_bloginfo("admin_email"),get_bloginfo('name'),$email_title,$last_message_email);
					}
